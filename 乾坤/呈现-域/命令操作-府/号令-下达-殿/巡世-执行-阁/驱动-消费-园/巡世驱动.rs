...
pub fn 调度驱动() -> String {
    ...
    let 涉及路径: Vec<&Path> = 候选.依据
        .split_whitespace()
        .filter(|s| s.contains('/') || s.contains('\\'))
        .map(Path::new)
        .collect();
    let 工作区根 = 状态目录路径.parent().map(Path::new).unwrap_or(Path::new("."));
    ...
    format!(
        "调度驱动完成\n来源：{}\n消费候选：{}\n本质档位：{:?}\n剩余候选：{} 条\n投递回执：\n{}",
        来源, 候选.目标, 候选.本质档位, 状态.巡世候选池.len(), 回执
    )
}
...
fn 档位优先选下一个(候选池: &[tianting_fu::巡世候选]) -> Option<&tianting_fu::巡世候选> {
