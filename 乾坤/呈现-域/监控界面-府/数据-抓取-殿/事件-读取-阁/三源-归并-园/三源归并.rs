...
fn 装配事件流(值: serde_json::Value) -> 白箱事件 {
    ...
    白箱事件 {
        ...
        任务线id,
        轮次,
        思考链: None, // §13.f.7a 思考链仅从 jiance_fu 观测记录还原，事件流不含
    }
}
...
fn 装配观测记录(值: serde_json::Value) -> 白箱事件 {
    ...
    let 轮次 = 值
        ...
        .and_then(|p| p.get("轮次"))
        .and_then(|v| v.as_u64());
    // §13.f.7a 思考链还原：从 jiance_fu 记回复 入库的 附加.思考 提取。
    // 仅在回复思考/回复内容域提取，避免其他域误读 附加.思考 字段。
    let 思考链 = match 域.as_str() {
        "回复思考" | "回复内容" => 附加
            .and_then(|v| v.get("思考"))
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
...
fn 装配识海记录(值: serde_json::Value) -> 白箱事件 {
    ...
    白箱事件 {
        ...
        任务线id,
        轮次,
        思考链: None, // §13.f.7a 思考链仅从 jiance_fu 观测记录还原
    }
}
...
/// §13.f.7a 思考链还原：从 jiance_fu 附加.思考 字段提取到白箱事件.思考链。
#[test]
fn 装配观测记录_思考链从附加还原() {
    let 值: serde_json::Value = serde_json::from_str(
            r#"{"时间戳":1,"域":"回复思考","接口":"调用","角色":"执行","载荷":{"内容":"回复","附加":{"思考":"推理内容"}}}"#,
        ).unwrap();
    let 事件 = 装配观测记录(值);
    assert_eq!(事件.思考链, Some("推理内容".to_string()));
}

/// §13.f.7a 思考链缺省：附加无 思考 字段时 白箱事件.思考链 为 None。
#[test]
fn 装配观测记录_无附加思考时None() {
    let 值: serde_json::Value = serde_json::from_str(
            r#"{"时间戳":1,"域":"回复思考","接口":"调用","角色":"执行","载荷":{"内容":"回复","附加":{"模型":"MiniMax-M3"}}}"#,
        ).unwrap();
    let 事件 = 装配观测记录(值);
    assert_eq!(事件.思考链, None);
}

/// §13.f.7a 思考链仅在回复思考/回复内容域提取，其他域忽略附加.思考。
#[test]
fn 装配观测记录_非回复域忽略附加思考() {
    // 工具调用域即使附加有 思考 字段，也不应提取（防误读）
    let 值: serde_json::Value = serde_json::from_str(
            r#"{"时间戳":1,"域":"工具调用","接口":"工具循环::执行工具","角色":"执行","载荷":{"内容":"cargo build","附加":{"思考":"误判"}}}"#,
        ).unwrap();
    let 事件 = 装配观测记录(值);
    assert_eq!(事件.思考链, None, "工具调用域不应提取思考");
}

/// §13.f.7a 思考链空字符串不入：附加.思考 为空字符串时不应入白箱事件。
#[test]
fn 装配观测记录_思考空字符串不入() {
    let 值: serde_json::Value = serde_json::from_str(
            r#"{"时间戳":1,"域":"回复思考","接口":"调用","角色":"执行","载荷":{"内容":"回复","附加":{"思考":""}}}"#,
        ).unwrap();
    let 事件 = 装配观测记录(值);
    assert_eq!(事件.思考链, None, "空字符串不入");
}
