...
pub enum 长河事件 {
    ...
    /// 回复收尾
    回复结束 { 消息id: String },
    /// 己方发言（用户消息入河，刷新/恢复后历史可见）
    己言 { 消息id: String, 文: String },
    /// 思考流开始（折叠呈现）
    思考开始 { 消息id: String },
    /// 思考正文增量
...
pub fn 归约(前: &会话视图, 事: &长河事件) -> 会话视图 {
    ...
    match 事 {
        ...
        长河事件::己言 { 消息id, 文 } => {
            let 消息 = 开消息(&mut 后, 消息id, 方向::己方, "界主");
            消息.段.push(段视图::正文 { 文: 文.clone() });
        }
        长河事件::思考开始 { 消息id } => {
...
mod 归约验证 {
    ...
    fn 己言入河为界主己方正文() {
        let 视 = 逐条归约(&[
            长河事件::己言 { 消息id: "u1".into(), 文: "你好".into() },
        ]);
        assert_eq!(视.消息.len(), 1);
        assert_eq!(视.消息[0].方向, 方向::己方);
        assert_eq!(视.消息[0].角色, "界主");
        assert_eq!(视.消息[0].段, vec![段视图::正文 { 文: "你好".into() }]);
    }
