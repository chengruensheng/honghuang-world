...
impl 智能体 {
    ...
    pub fn 运行从(&self, 任务: String, 已存消息: Vec<对话消息>) -> Result<String> {
        ...
        for 轮次 in 0..self.最大轮数 {
            ...
            for 调用 in &响应.工具调用 {
                ...
                // 命令安全审计（旁路，不改控制流）：执行前以空上下文让 LLM 判定命令风险，只记录不拦截
                self.执行前审计(轮次, 调用);
    ...
    /// 执行前命令安全审计（旁路观察，不改控制流）：仅对「运行命令」触发，以空上下文独立会话
    /// 让 LLM 判定该命令在沙箱内执行是否安全，结论发一条事件流摘要，并在工作区
    /// `.审计/命令审计.jsonl` 追加一行完整记录（未注入工作区根时只发事件不落盘）。
    /// **任何失败都只告警不阻断**：本能力当前只做数据积累，放行/拒绝仍由执行器白名单唯一决定。
    fn 执行前审计(&self, 轮次: usize, 调用: &工具调用) {
        if 还原工具名(&调用.名称) != 运行命令 {
            return;
        }
        let 命令 = serde_json::from_str::<serde_json::Value>(&调用.参数)
            .ok()
            .and_then(|参数| {
                参数
                    .get(参数键_命令)
                    .or_else(|| 参数.get(参数键_命令_英))
                    .and_then(|值| 值.as_str())
                    .map(|文| 文.to_string())
            });
        let 命令 = match 命令 {
            Some(命令) if !命令.trim().is_empty() => 命令,
            _ => return,
        };
        match super::命令审计::审计命令(&self.对话器, &命令) {
            Ok(结论) => {
                let 摘要 = format!(
                    "【命令安全审计】风险={} 安全={} 命令={} 理由={}",
                    结论.风险,
                ...
                self.发事件(轮次, 开发事件类型::思考, "", &摘要);
                if let Some(根) = &self.工作区根 {
                    super::命令审计::追加审计名册(&format!("{根}/.审计"), &命令, &结论);
                }
            }
            Err(失败) => tracing::warn!("命令安全审计失败（旁路，不影响执行）: {失败}"),
        }
    }
...
///
/// 错误文案必须带上「可接受键」与「实收键」：只报「缺少参数: 命令」时模型无从得知该改什么，
/// 会反复用同一份空参数重试直到熔断（2026-09-14 实证：#51 准圣连续 3 次 `运行命令` 传 `{}`）。
...
/// 列出本轮实收参数的顶层键，用于错误文案回喂（空对象显示「无」，非对象显示原文）
fn 实收参数键(参数: &serde_json::Value) -> String {
    match 参数 {
        serde_json::Value::Object(表) => {
            if 表.is_empty() {
                "无".to_string()
            } else {
                表.keys().cloned().collect::<Vec<_>>().join(" / ")
            }
        }
        _ => 截断(&参数.to_string(), 80),
    }
}
