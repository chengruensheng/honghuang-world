...
const 系统提示: &str = "你是一个自主开发智能体，在指定工作区内完成开发任务。可调用「读文件」「写文件」「运行命令」「列目录」「按名找文件」「搜索内容」「精确编辑」「删除文件」「任务清单」九个工具：列目录看一层条目，按名找文件用 glob 模式（*、**、?）找文件，搜索内容按关键词递归检索文本（返回 路径:行号:内容），精确编辑把文件中唯一匹配的旧串替换为新串（多处匹配会报错，需提供更精确上下文），删除文件删除工作区内文件（清理临时/备份产物，如写文件/精确编辑自动产生的 .bak 备份），任务清单用数组维护待办/进行中/已完成的多步计划。运行环境是 Windows，命令须用 cmd 语法（列目录用 dir、查看文件用 type、构建测试用 cargo）。每步先思考再行动；工具调用失败要读取错误信息并修正参数重试，不得在工具失败时直接宣告任务完成；任务完成后停止调用工具并给出简短说明。每次修改代码或配置后必须运行 cargo build 与 cargo test 验证通过，验证失败须读取错误并修复，不得跳过验证。跨文件或跨 crate 改动时：新增依赖加到实际使用它的 crate 的 Cargo.toml；修改函数签名后须同步更新所有调用点。";
...
impl 智能体 {
    ...
    fn 发事件(&self, 轮次: usize, 类型: 开发事件类型, 工具名: &str, 内容: &str) {
        if let Some(回调) = &self.事件回调 {
            // 工具输出是过程噪音，200 字节够用；答复是结论，须给足额度。
            let 上限 = if matches!(&类型, 开发事件类型::任务答复) { 答复内容上限 } else { 事件内容上限 };
            let 事件 = 开发事件 {
                轮次,
                类型,
                工具名: 工具名.to_string(),
...
fn 工具定义() -> Vec<serde_json::Value> {
    vec![
        ...
        单参函数_可选(列目录, "列出工作区内目录下条目（一层，区分目录/文件）；路径可缺省，缺省列工作区根目录", 参数键_路径, "相对工作区的目录路径，缺省为根目录"),
        ...
        单参函数(删除文件, "删除工作区内的单个文件（清理临时/备份产物，如 .bak）", 参数键_路径, "相对工作区的文件路径"),
        任务清单函数(),
    ]
}
