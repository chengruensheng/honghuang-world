...
use tc_task::{TaskBoard, TaskStatus, 五行层级, 状态归属角色, 状态层级标签};
...
pub struct 五层协作驱动器 {
    ...
    /// 透传（任务id, 角色名, 阶段层级名, 开发事件）——层级名取承接状态经 状态层级标签 的权威映射，
    /// 会话落盘后即可按层结构化统计，不必对事件内容做中文文本匹配
    过程回调: Option<Arc<dyn Fn(u64, &str, &str, &开发事件) + Send + Sync>>,
...
impl 五层协作驱动器 {
    ...
    /// 链式注入过程事件回调：智能体循环每步通知外部（任务id, 角色名, 阶段层级名, 开发事件）
    pub fn 设置过程回调(mut self, 回调: Arc<dyn Fn(u64, &str, &str, &开发事件) + Send + Sync>) -> Self {
        self.过程回调 = Some(回调);
        self
    }
    ...
    pub fn 执行一轮_恢复(&self, 恢复: Option<(u64, Vec<对话消息>)>) -> Result<驱动结果> {
        ...
        // 注入过程事件回调：把智能体循环每步事件通知外部（带任务id、角色名与阶段层级名）。
        // 层级由承接状态经 状态层级标签 得出（与阶段事件同源），使会话事件自带结构化层标签。
        if let Some(回调) = &self.过程回调 {
            let 任务id = 快照.id;
            let 角色名 = 角色.名称().to_string();
            let 层级名 = 状态层级标签(快照.状态).名().to_string();
            let 回调克隆 = 回调.clone();
            智能体 = 智能体.设置事件回调(Arc::new(move |事件| {
                回调克隆(任务id, &角色名, &层级名, 事件);
            }));
        }
        ...
        let 产出 = 解析并构造带重试(&角色, 快照.状态, &答复, &提示, &智能体, &核验器, self.工作区根.as_deref())?;
        ...
        let (实际新状态, 回退信息) = {
            ...
            if 下一状态 == TaskStatus::待修复 && 角色 == AgentRole::准圣 {
                ...
                // 机器核验被推翻时，追溯现象必须用核验摘要（真实机器证据）——模型答复已被推翻，
                // 拿它追溯会丢失真实错误，让词面启发式乱判根源（2026-09-13 实证）。
                看板.提交任务(快照.id, 角色, TaskStatus::待修复)?;
                let 现象 = match &产出.核验 {
                    Some(结论) if !结论.通过 => 结论.摘要.clone(),
                    _ => 答复.clone(),
                };
                let (根源层级, 错误描述) = 追溯根源(&快照, &现象);
        ...
        // 6d. 任务终态（清理完成 / 已确认无解等）：清除临时规则，规则不外溢到后续任务
        if 实际新状态.是终态() {
            if let Some(认知) = &self.认知 {
                认知.清除临时规则();
                tracing::info!("任务 #{} 已终态（{:?}），临时规则已清除", 快照.id, 实际新状态);
            }
        }
