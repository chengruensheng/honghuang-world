    ...
    let 阶段要求 = match 角色 {
        ...
        AgentRole::大罗金仙 => {
            "你的任务是【实现】。在工作区实际实现设计（可用读文件/写文件/运行命令/搜索内容/精确编辑等工具），\
             ...
             {\"代码变更\":[{\"文件路径\":\"\",\"变更类型\":\"\",\"摘要\":\"\"}],\
             \"工具调用\":[{\"工具名\":\"\",\"参数\":\"\",\"结果摘要\":\"\",\"时间戳\":0}],\
             \"自检\":{\"通过\":true,\"边界合规\":true,\"契约合规\":true,\"问题\":[]}}\n\
             关键约束：「工具调用」若填写，每个元素的字段名**只能**是 工具名/参数/结果摘要/时间戳 四个，\
             缺任一字段都会导致整份文档解析失败；参数写该次调用的入参原文（字符串），\
             结果摘要写返回要点（字符串），时间戳填该次调用的秒级时间戳（数值，不确定时填 0）。\
             不需要工具调用清单时可把「工具调用」写成 []，切勿改字段名。"
        }
        AgentRole::准圣 => {
