//! 对外接口-殿/看板接口-阁/看板-处理-园：任务看板发布与流转接口。
...
pub async fn 看板列表<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
...
pub async fn 看板查询<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Path(id): Path<u64>,
...
/// POST /api/board — 发布任务
pub async fn 看板发布<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Json(请求): Json<发布任务请求>,
...
pub async fn 看板承接<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Path(id): Path<u64>,
    Json(请求): Json<承接任务请求>,
...
pub async fn 看板提交<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Path(id): Path<u64>,
    Json(请求): Json<提交任务请求>,
) -> Result<StatusCode, (StatusCode, String)> {
