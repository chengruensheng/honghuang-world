//! 对外接口-殿/开发接口-阁/受理-处理-园：开发任务受理、道祖对话与执行台事件接口。

use axum::{Json, extract::Query, http::StatusCode, response::sse::{Event, KeepAlive, Sse}};
...
use crate::{
    接口状态, 受理错误响应, 事件游标,
    受理依赖, 受理开发任务, 受理失败, 确认发布对齐需求, 事件记录, 会话阶段, 需求摘要, 思考链过滤器,
};
...
pub async fn 受理开发任务接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Json(请求): Json<开发受理请求>,
...
pub async fn 事件查询<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Query(游标): Query<事件游标>,
) -> Json<开发事件响应> {
    let 台 = &状态.开发执行台;
...
pub async fn 停止执行<M: Send + Sync + 'static>(状态: 接口状态<M>) -> Json<停止响应> {
    let 中断 = 状态.开发执行台.中断();
    if !中断 {
...
pub async fn 更新工作区<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Json(请求): Json<工作区请求>,
...
pub async fn 道祖对话接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Json(请求): Json<道祖对话请求>,
...
/// 并发超限返回 429。
pub async fn 道祖对话流式接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Json(请求): Json<道祖对话请求>,
...
pub async fn 道祖确认接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
