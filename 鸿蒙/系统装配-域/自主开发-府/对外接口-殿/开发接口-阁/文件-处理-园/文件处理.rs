//! 对外接口-殿/开发接口-阁/文件-处理-园：文件清单与内容读取、规则条文写入。
//!
//! 分类规则（互斥，按优先级）：
//! 1. 架构：文件名含「架构」或「蓝图」；
//! 2. 门禁：文件名含「门禁」；
...
//!
//! 扫描根取对外接口服务状态.扫描根（默认 ./），递归收集，跳过编译产物与版本控制目录。
...
pub async fn 文件清单接口<M: Send + Sync + 'static>(状态: 接口状态<M>) -> Json<文件清单响应> {
...
pub async fn 文件内容接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Query(请求): Query<内容请求>,
...
pub async fn 规则写入接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Json(请求): Json<规则写入请求>,
