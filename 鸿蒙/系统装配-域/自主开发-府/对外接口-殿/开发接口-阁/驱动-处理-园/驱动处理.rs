//! 对外接口-殿/开发接口-阁/驱动-处理-园：看板驱动、会话回放与驱动事件流接口。
...
pub async fn 看板驱动接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
...
/// 未装配 → 503；运行中 → 409。
pub async fn 看板驱动到空闲接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
...
pub async fn 看板驱动事件接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Query(游标): Query<事件游标>,
) -> Json<驱动事件响应> {
    let 台 = &状态.看板驱动台;
...
pub async fn 看板驱动状态接口<M: Send + Sync + 'static>(状态: 接口状态<M>) -> Json<看板驱动状态> {
    Json(状态.看板驱动台.当前状态())
}
...
pub async fn 看板驱动过程接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Query(游标): Query<事件游标>,
) -> Json<驱动过程响应> {
    let 台 = &状态.看板驱动台;
...
pub async fn 会话清单接口<M: Send + Sync + 'static>(状态: 接口状态<M>) -> Json<会话清单响应> {
    Json(会话清单响应 {
        会话: 状态.看板驱动台.会话清单(),
    })
}
...
pub async fn 会话回放接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Path(会话id): Path<u64>,
...
pub async fn 会话恢复接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Path(会话id): Path<u64>,
    Json(请求): Json<会话定向请求>,
) -> Result<Json<驱动受理响应>, (StatusCode, Json<受理错误响应>)> {
...
pub async fn 会话分叉接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Path(会话id): Path<u64>,
    Json(请求): Json<会话分叉请求>,
...
pub async fn 看板驱动过程流接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Query(游标): Query<事件游标>,
...
pub async fn 看板驱动阶段流接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Query(游标): Query<事件游标>,
) -> Result<Sse<impl Stream<Item = Result<Event, Infallible>>>, StatusCode> {
    let Ok(许可) = 状态.sse信号量.clone().try_acquire_owned() else {
...
pub async fn 看板驱动过程流_agui接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Query(游标): Query<事件游标>,
) -> Result<Sse<impl Stream<Item = Result<Event, Infallible>>>, StatusCode> {
    let Ok(许可) = 状态.sse信号量.clone().try_acquire_owned() else {
...
pub async fn 看板驱动阶段流_agui接口<M: Send + Sync + 'static>(
    状态: 接口状态<M>,
    Query(游标): Query<事件游标>,
) -> Result<Sse<impl Stream<Item = Result<Event, Infallible>>>, StatusCode> {
    let Ok(许可) = 状态.sse信号量.clone().try_acquire_owned() else {
