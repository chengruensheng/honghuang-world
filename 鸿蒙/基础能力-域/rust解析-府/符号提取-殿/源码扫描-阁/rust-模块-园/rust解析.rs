...
//!
//! 下钻范围：`impl` 块与内联 `mod` 会继续进入，方法、关联项、嵌套模块内的符号都登记。
//! 不下钻函数体——函数体内的嵌套条目属局部作用域，纳入只会给"死代码"判定添噪声。
//!
...
/// 只有 `impl` 块与内联 `mod` 体内还装符号；`mod x;` 声明没有 body，自然止步
fn 可下钻(节点: tree_sitter::Node) -> bool {
    matches!(节点.kind(), "impl_item" | "mod_item")
}
