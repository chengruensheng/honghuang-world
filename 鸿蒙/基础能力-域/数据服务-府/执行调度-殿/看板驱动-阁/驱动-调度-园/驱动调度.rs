...
impl 看板驱动台 {
    ...
    pub fn 启动执行一轮(self: &Arc<Self>) {
        ...
        std::thread::spawn(move || {
            let 结果 = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| 驱动器.执行一轮()))
                .unwrap_or_else(|_| Err(hm_error::Error::Other("看板驱动线程异常终止".into())));
