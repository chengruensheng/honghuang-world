...
impl 本地执行器 {
    ...
    fn 运行命令_超时(&self, 命令: &str, 超时秒: u64) -> Result<String> {
        ...
        // 引号外 `^` 单独给出准确原因：笼统报「命令不在白名单」会把模型引向
        // 「cargo/命令被白名单限制」的错误诊断，并随验收记录一路传播（2026-09-14 实证）。
        if 引号外含脱字符(&实际命令) {
            return Err(Error::危险命令(format!(
                "命令含引号外的 cmd 转义符 '^'（可伪装命令名绕过白名单），已拒绝；\
                 正则需用 '^' 时请写在双引号内（如 findstr /v \"^$\"）: {实际命令}"
            )));
        }
        if 命令不在白名单(&实际命令) {
...
/// 引号外是否含 cmd 转义符 `^`。
///
/// `^` 是 cmd 的转义符：`d^el` 会被 cmd 当作 `del` 执行，故**引号外**一律拒绝。
/// 双引号内 `^` 是普通字符（cmd 在引号态不解释元字符、也不剥离它），
/// 一律拒绝会误杀 `findstr /v "^$"` 这类正当写法——2026-09-14 实证：镜像验证任务
/// 因此无法运行 cargo build/test，且误报「命令不在白名单」把模型引向「cargo 被限制」的错误诊断。
/// 引号内分隔符仍会被 拆命令段 按原样切分并单独校验，故放行引号内 `^` 不引入绕过面。
fn 引号外含脱字符(命令: &str) -> bool {
    let mut 引号内 = false;
    for 字符 in 命令.chars() {
        match 字符 {
            '"' => 引号内 = !引号内,
