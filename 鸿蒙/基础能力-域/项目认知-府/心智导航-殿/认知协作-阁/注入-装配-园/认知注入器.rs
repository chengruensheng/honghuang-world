...
use crate::{消息角色, 注入器, 上下文库, 三态存储, 图谱, 心智地图, 检索器, 检索源, 检索决策记录, 维度, 压缩器, 纠错引擎, 上下文压缩阈值};

/// 认知注入：三态（图谱/格位/临时上下文）接入智能体运行上下文的装配句柄。
///
/// 与同阁的 注入器 不同：本结构持有共享所有权句柄（Arc<Mutex>），
/// 供智能体循环在每次决策前 推（格位常驻）/拉（图谱按需）/流（最近过程）
...
impl 认知注入 {
    ...
    /// 供客户端认知问答接口消费（决策轨迹可见）；答复可选 LLM 组装。
    pub fn 检索决策(
        &self,
        问题: &str,
