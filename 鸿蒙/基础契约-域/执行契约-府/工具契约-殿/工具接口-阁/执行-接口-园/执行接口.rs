...
///
/// **读写分离边界**：写操作（写文件/精确编辑/删除文件）与工作区命令**严格锁定工作区**；
/// 读操作（读文件/列目录/按名找文件/搜索内容）可越出工作区探索本体代码——`~/` 前缀指向项目根，
/// 绝对路径须落在只读根集合内，且命中读黑名单（`.git`/`.env`）时拒绝。
pub trait 执行器: Component {
    /// 读取文件完整文本。普通相对路径 = 工作区内；`~/路径` = 项目根（只读探索）
    fn 读文件(&self, 路径: &str) -> Result<String>;
    /// 写入内容到工作区内文件（覆盖）。只能写工作区，`~/` 与绝对路径一律拒绝
    fn 写文件(&self, 路径: &str, 内容: &str) -> Result<()>;
    /// 在工作区内运行命令，返回标准输出；`~ 命令` 前缀则在项目根下运行（仅只读白名单）
    fn 运行命令(&self, 命令: &str) -> Result<String>;
    /// 列出目录下条目（一层，非递归），区分目录/文件，按名排序；`~/路径` 列项目根
    fn 列目录(&self, 路径: &str) -> Result<String>;
    /// 按 glob 模式（`*`/`**`/`?`）递归匹配文件，返回路径列表；`~/` 开头则在项目根匹配
    fn 按名找文件(&self, 模式: &str) -> Result<String>;
    /// 递归搜索文本文件内容，返回匹配行（路径:行号:内容）；`~/关键词` 则搜项目根
    ...
    /// 以指定超时（秒）运行命令：供机器核验类长耗时命令（编译/测试）使用——
    /// 这类命令显著长于普通工具命令，沿用默认超时会被误判为超时失败。
    /// 默认实现忽略超时参数并转发 `运行命令`，使既有实现（测试 mock / 其它执行器）无需改动。
    fn 运行命令_限时(&self, 命令: &str, _超时秒: u64) -> Result<String> {
        self.运行命令(命令)
    }
}