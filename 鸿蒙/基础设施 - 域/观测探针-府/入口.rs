...
/// 从环境变量 `观测完整记录` 读取，设为 "开" 时记录完整内容，否则只记摘要。
...
///
/// §13.f.7a 思考链入库契约：思考参数非空时入载荷 `附加.思考`（独立字段，便于消费方还原），
/// 载荷.内容 保留原 `【回复】/【思考】` 格式便于回退消费方（不改既有 JSON 形状）。
pub fn 记回复(
    角色: 观测角色,
    接口: &str,
    思考: &str,
    ...
    let 思考非空 = !思考.trim().is_empty();
    let 合并附加 = match 附加 {
        Some(mut 值) => {
            // 思考非空时入 附加.思考；空时不动（避免污染）
            if 思考非空 {
                if let Some(obj) = 值.as_object_mut() {
                    obj.insert(
                        "思考".to_string(),
                        serde_json::Value::String(思考.to_string()),
                    );
                }
            }
            Some(值)
