...
/// 涉及路径护栏：目标路径必须落在要求书涉及路径内（目标 == 涉及路径，或以涉及路径为目录前缀）。
...
pub fn 校验涉及路径(原始: &str, 涉及路径: &[String]) -> Result<(), String> {
    ...
    let 在内 = 涉及路径.iter().any(|涉及| {
        let 涉及 = 涉及.replace('\\', "/");
        目标 == 涉及 || 目标.starts_with(&format!("{涉及}/"))
    });
    if 在内 {
        Ok(())
    } else {
        Err(format!(
            "涉及路径外拒写：{目标}（只允许落在要求书涉及路径内：{}）——如需改其他文件，请界主把目标路径写进涉及路径",
            涉及路径.join("、")
        ))
    }
}
...
mod 测试 {
    ...
    fn 涉及路径_文件精确与目录树内放行() {
        ...
        assert!(校验涉及路径("乾坤/呈现-域/命令操作-府/观览-查询-殿/世界-观览-阁/流式-回放-园", &涉及).is_ok(), "涉及文件所在目录可建");
    }
    ...
    fn 涉及路径_越界拒绝() {
        ...
        // 相似前缀不可误放行：涉及 流式-回放-园，不能写 流式-回放-园外 的 流式-回放-园X。
