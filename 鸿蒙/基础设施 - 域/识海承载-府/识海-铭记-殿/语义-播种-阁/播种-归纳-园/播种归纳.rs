...
use crate::{依赖图, 扫描排除项, 来源, 格位, 模型存储, 渲染播种提示, 记录};
...
pub fn 播种降级(
    ...
    // 白箱观测：播种整链进入归因角色（含两条 LLM 归纳路径）。
    let _观测守卫 = 进入观测(观测角色::归因, None, None, None);
    if !格位.推荐位置.is_empty() {
        let 候选 = 根.join(&格位.推荐位置);
        if 候选.is_file() {
            let 素材 =
                std::fs::read_to_string(&候选).map_err(|错误| format!("读推荐位置失败: {错误}"))?;
    ...
    if let Some(首个) = 找首个源文件(根, &排除项) {
        let 内容 =
            std::fs::read_to_string(&首个).map_err(|错误| format!("读源文件失败: {错误}"))?;
        let 摘要: String = 内容.lines().take(3).collect::<Vec<_>>().join("\n");
        let 记录 = 记录::新(
            &格位.名字,
            &摘要,
            &format!("从「{}」扫描", 首个.display()),
            "代码",
        );
        存储.写记录(&记录)?;
        return Ok(播种结果::已归纳(摘要));
...
fn 找首个源文件(根: &Path, 排除项: &[String]) -> Option<std::path::PathBuf> {
    let Ok(条目们) = std::fs::read_dir(根) else {
        return None;
    };
    for 条目 in 条目们.flatten() {
        ...
        } else if 路径.extension().is_some_and(|扩展名| 扩展名 == "rs") {
            return Some(路径);
        }
    }
    None
...
fn 递归搜集源文件(
    根: &Path, 目录: &Path, 排除项: &[String], 文件们: &mut Vec<String>
) {
    let Ok(条目们) = std::fs::read_dir(目录) else {
        return;
    };
...
pub fn 模型播种(
    ...
    if !文档们.is_empty() {
        ...
        let 回答 = 界主.询问(
            &格位.名字,
            &format!("归纳未通过审阅，请界主给出「{}」的正确内容", 格位.名字),
        );
        if !回答.is_empty() {
...
/// 可操作规则格位注入（设计稿 §4.2 规则7 / 项目心智模型 §6.5）：
/// 读 `细则·解读`（规则范畴）格位链头，格式化「【可操作规则】{内容}」注入各环节提示词首行。
/// 规则从格位读、与代码解耦：改格位即全局生效，不需重编号令。
/// 空记录/无有效记录 → 零开销返回空串，不注入。调用失败只记警告，不阻断主流程。
pub fn 注入规则(存储: &模型存储, 格位名: &str) -> String {
    let 记录们 = match 存储.读格位(格位名) {
        Ok(记录们) => 记录们,
        Err(错误) => {
            error!(格位名, "读可操作规则格位失败：{错误}");
            return String::new();
        }
    };
    // 链尾最新一条非失效记录（读格位按写入顺序，链头 = 时间最新）。
    if let Some(记录) = 记录们.into_iter().rev().find(|记录| !记录.失效) {
        let 内容 = 记录.内容.trim();
        if !内容.is_empty() && 内容 != "不确定" {
            debug!(格位名, 内容长度 = 内容.len(), "可操作规则已注入");
            return format!("【可操作规则·{格位名}】\n{内容}\n");
        }
    }
    String::new()
}
...
mod 测试 {
    ...
    fn 临时代偿_人类已录格位零成本跳过() {
        let 存储 = 临时存储();
        // 预写所有来源=人类的格位记录：临时代偿应全部跳过（返回 0，
        // 不触发 LLM 调用；若误触发会因空密钥地址报错，此测试通过即证明未触发）。
        // 此前仅预写「初心·使命」一条，其余人类格位仍会递归搜集 temp 目录文档并
        // 尝试调用模型（空配置致网络超时），单测耗时 146 秒；预写全部人类格位后
        // 跳过检查全部命中，毫秒级完成且断言更严格（所有人类格位均跳过）。
        for 格位 in crate::全部格位()
            .iter()
            .filter(|格位| 格位.来源 == crate::来源::人类)
        {
            存储
                .写记录(&记录::新(&格位.名字, "人类已填内容", "界主录入", "人类"))
                .unwrap();
        }
