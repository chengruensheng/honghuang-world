...
use crate::世界结果;
use crate::识海_纳藏_殿::模型_存储_阁::类型_声明_园::{
    是_测试文件, 测试覆盖判定
};
...
pub fn 扫描依赖边(根目录: &Path) -> 世界结果<依赖图> {
    ...
    // 第一遍：符号定义 + 解释（波及暂空）。同时记录每文件的符号数 + 测试覆盖判定，供第二遍补占位档案。
    // §19.4.1 + §8.5：依赖图已读全文 → 零额外 IO 算 测试覆盖判定 字段值。
    // - 路径命中测试域（是_测试文件）→ 是测试
    // - 代码内含 #[test]/mod 测试 命中（扫测试符号）→ 是测试
    // - 其他 → 保持 未判定（依赖图::default，serde 兜底）
    ...
    for 文件 in &文件们 {
        ...
        let mut 本文件符号数 = 0;
        // 测试符号命中：先算整个文件，再在 push 档案时设字段（避免每行重复扫）。
        let (测试标记, 测试模块) = 依赖图::扫测试符号(&行们);
        let 文件含测试符号 = 测试标记 > 0 || 测试模块 > 0;
        let 路径是测试 = 是_测试文件(&相对);
        for (序号, 行) in 行们.iter().enumerate() {
            if let Some((名, 签名)) = 提取符号签名(行) {
                ...
                let 定义体 = 提取定义块(&行们, 序号);
                let mut 档案 = 符号档案::新(&项目名, &模块, &相对, &名, &定义体, &签名, &解释);
                // 业务文件（pub 符号定义体）：通常本身不是测试；pub 符号定义体不含
                // #[test] → 字段保持 未判定（接单门闭包走 4 层瀑布判）。
                // 业务文件若含 #[test]（罕见，内嵌单测） → 是测试。
                if 路径是测试 || 文件含测试符号 {
                    档案.测试覆盖判定 = 测试覆盖判定::是测试;
                }
                图.档案们.push(档案);
            }
        }
    ...
    for 文件 in &文件们 {
        ...
        // 占位档案通常是纯测试文件（无 pub 符号）→ 天然是测试。
        // 路径命中测试域或代码含 #[test]/mod 测试 标记 → 是测试；
        // 否则保持 未判定（罕见：非测试文件无 pub 符号，如 main.rs/bin 入口）。
        let (测试标记, 测试模块) = 依赖图::扫测试符号(&内容.lines().collect::<Vec<_>>());
        if 是_测试文件(&相对) || 测试标记 > 0 || 测试模块 > 0 {
            占位档案.测试覆盖判定 = 测试覆盖判定::是测试;
        }
        图.档案们.push(占位档案);
    }
