...
impl 回滚垫 {
    ...
    pub fn 备份(&self, 任务id: &str, 路径: &str) -> 世界结果<()> {
        let 任务id = 分组(任务id);
    ...
    pub fn 撤销(&self, 任务id: &str) -> 世界结果<u32> {
        let 任务id = 分组(任务id);
        let 任务目录 = self.目录.join(任务id);
        if !任务目录.is_dir() {
            return Ok(0);
    ...
    pub fn 撤销任务前缀(&self, 任务id: &str) -> 世界结果<u32> {
        let 前缀 = 分组(任务id);
        if !self.目录.is_dir() {
            return Ok(0);
        }
    ...
    fn 撤销组(&self, 任务id: &str, 任务目录: &std::path::Path) -> 世界结果<u32> {
        let mut 条目们 = Vec::new();
    ...
    /// 撤销当前任务（用线程当前任务id）。
    pub fn 撤销当前(&self) -> 世界结果<u32> {
        self.撤销(&当前任务())
    }

    /// 清理：任务成功结束后丢弃存档。
    pub fn 清理(&self, 任务id: &str) -> 世界结果<()> {
        let 任务id = 分组(任务id);
        let 任务目录 = self.目录.join(任务id);
        if 任务目录.is_dir() {
