...
impl 依赖图 {
    /// 保存在工作区（.上下文/依赖图.json）。
    pub fn 保存在工作区(&self, 工作区: &工作区) -> 世界结果<()> {
        self.保存(工作区.依赖图路径())
    }

    /// 保存到指定路径。
    pub fn 保存(&self, 路径: impl AsRef<Path>) -> 世界结果<()> {
        let 文本 = serde_json::to_string_pretty(self)
            .map_err(|错误| format!("序列化依赖图失败: {错误}"))?;
        debug!(路径 = %路径.as_ref().display(), "依赖图已保存");
        std::fs::write(路径.as_ref(), 文本).map_err(|错误| format!("保存依赖图失败: {错误}").into())
    }

    /// 从工作区加载（.上下文/依赖图.json）。
    pub fn 加载自工作区(工作区: &工作区) -> 世界结果<依赖图> {
        Self::加载(工作区.依赖图路径())
    }

    /// 从指定路径加载（不存在则返回空图）。
    /// 加载后归一化路径字段为正斜杠：兼容旧 json 反斜杠存储，归一后查询免 replace。
    pub fn 加载(路径: impl AsRef<Path>) -> 世界结果<依赖图> {
        let 路径 = 路径.as_ref();
        if !路径.exists() {
