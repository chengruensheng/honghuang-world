...
use crate::{可见工具, 格位, 顺序档位, 范畴, 模型存储};
...
mod 测试 {
    ...
    fn 元数据层化_固定规则段注入规则格位() {
        ...
        存储.写记录(&记录::新("细则·解读", "全中文输出纪律：所有输出用简体中文", "界主规则", "人类")).unwrap();
        // 造一个 非规则 中间档格位（如 结构 = 范畴::世界），不应进固定规则段。
        let 世界格位 = 格位::新("结构", 范畴::世界, "目录组织", "代码", 固化度::权, 共享度::共享, 顺序档位::中间);
        存储.写记录(&记录::新("结构", "目录快照", "扫描", "代码")).unwrap();

        let 背景 = 元数据层化(&存储, "多宝", &[规则格位, 世界格位], 100_000).unwrap();
        assert!(背景.contains("固定规则"), "应含固定规则段：{背景}");
        assert!(背景.contains("全中文输出纪律"), "应注入规则格位内容：{背景}");
        assert!(!背景.contains("目录快照"), "非规则中间档(世界)不应进固定规则段：{背景}");
        let _ = std::fs::remove_dir_all(&目录);
    }
    ...
    fn 元数据层化_固定规则段空记录不注入() {
        ...
        let 存储 = 模型存储::打开(&目录);
        let 规则格位 = 格位::新("细则·解读", 范畴::规则, "可操作规范", "LLM", 固化度::权, 共享度::共享, 顺序档位::中间);
        let 背景 = 元数据层化(&存储, "多宝", &[规则格位], 100_000).unwrap();
        assert!(!背景.contains("固定规则"), "空规则格位不应出固定规则段：{背景}");
        let _ = std::fs::remove_dir_all(&目录);
    }
}