...
/// 一次模型调用的 token 用量（含缓存命中/缓存写/推理，便于成本观测与对账）。
///
/// §13.f.7 token 累计展示：在 §9.3 基础上扩展分量（向后兼容，原 提示词/输出/缓存命中/总计 保留）。
/// 新增字段：缓存写（cacheWrite）、推理（reasoning_tokens，思考链消耗，与输出分开计量）。
...
pub struct 用量 {
    ...
    /// 总 token 数。
    pub 总计: u64,
    /// 缓存写入 token 数（cache_creation_input_tokens / cache_write_input_tokens）。
    /// §13.f.7 cacheWrite 分量，与缓存命中分开计量。
    #[serde(default, skip_serializing_if = "is_zero")]
    pub 缓存写: u64,
    /// 推理 token 数（completion_tokens_details.reasoning_tokens）。
    /// §13.f.7 reasoning 分量，思考链消耗，与输出分开计量。
    #[serde(default, skip_serializing_if = "is_zero")]
    pub 推理: u64,
}
...
#[cfg(test)]
mod 测试 {
    //! 用量累加接口契约测试：验证提示词/输出/缓存命中/缓存写/推理/总计六字段累加语义。
    use super::用量;

    #[test]
    fn 用量加法零值累加等于原值() {
        // 零值兜底：任意非零用量 + 用量::default() 六字段不变；反向零 + 非零也保持非零。
        let mut 累加 = 用量 {
            提示词: 100,
            输出: 50,
            缓存命中: 30,
        ...
        assert_eq!(累加.缓存写, 12, "非零 + 零：缓存写不变");
        assert_eq!(累加.推理, 234, "非零 + 零：推理不变");
        // 反向：零用量 + 非零用量 = 非零用量（六字段逐项不变）。
        ...
        累加2.加(&非零);
        assert_eq!(累加2, 非零, "零 + 非零：六字段全部继承非零值");
    }

    #[test]
    fn 用量加法六字段逐项累加() {
        // 对称累加：A.加(&B) 六字段 = A.x + B.x（提示词/输出/缓存命中/总计/缓存写/推理均独立累加，不串扰）。
        let mut a = 用量 {
            提示词: 100,
            输出: 50,
            缓存命中: 30,
        ...
        assert_eq!(a.缓存写, 36, "缓存写 = 12+24");
        assert_eq!(a.推理, 702, "推理 = 234+468");
        // 跨字段独立：单字段差异不应影响其他字段累加。
        let mut c = 用量 {
            提示词: 5,
            输出: 0,
            缓存命中: 0,
        ...
        assert_eq!(c.缓存写, 7, "跨字段：缓存写 0+7=7");
        assert_eq!(c.推理, 11, "跨字段：推理 0+11=11");
    }

    #[test]
    fn 用量加法链式累加多轮聚合() {
        // 链式累加：A.加(&B).加(&C) 六字段 = A+B+C（多任务聚合场景典型用法）。
        let mut a = 用量 {
            提示词: 10,
            输出: 5,
            缓存命中: 2,
        ...
        assert_eq!(a.缓存写, 111, "链式：1+10+100");
        assert_eq!(a.推理, 333, "链式：3+30+300");
    }

    /// §13.f.7 向后兼容：旧记录无 缓存写/推理 字段时反序列化默认零。
    #[test]
    fn 用量旧记录无新字段反序列化默认零() {
        let 旧 = r#"{"提示词":100,"输出":50,"缓存命中":30,"总计":180}"#;
        let 用: 用量 = serde_json::from_str(旧).unwrap();
        assert_eq!(用.提示词, 100);
        assert_eq!(用.输出, 50);
        assert_eq!(用.缓存命中, 30);
        assert_eq!(用.总计, 180);
        assert_eq!(用.缓存写, 0, "旧记录无缓存写默认零");
        assert_eq!(用.推理, 0, "旧记录无推理默认零");
    }

    /// §13.f.7 序列化时零值不输出（避免污染旧消费方）。
    #[test]
    fn 用量零值新字段不序列化() {
        let 用 = 用量 {
            提示词: 100,
        ...
        let 文本 = serde_json::to_string(&用).unwrap();
        let 值: serde_json::Value = serde_json::from_str(&文本).unwrap();
        assert!(
            值.get("缓存写").is_none() || 值["缓存写"].is_null(),
            "零值缓存写不序列化"
        );
        assert!(
            值.get("推理").is_none() || 值["推理"].is_null(),
            "零值推理不序列化"
        );
    }
}
