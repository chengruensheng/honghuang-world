...
use moxing_fu::{调用模型, 对话消息, 模型配置, 常规上限};
...
fn 改稿(要求: &要求书, 现稿: &设计方案, 意见文本: &str, 配置: &模型配置) -> 设计方案 {
    ...
    let 提示 = format!(
        "你是世界设计主笔。根据评审意见修改设计方案，只输出一个 JSON 对象，不要多余文字。\n\
         JSON 结构：{{\"设计\":\"设计思路\",\"拆解\":[{{\"目标\":\"子任务目标\",\"执行层角色\":[\"duobao\"],\"工作流\":\"L2_script|L3_program|L4_complex\"}}],\"自评\":\"设计为何满足验收标准（必填不可为空）\"}}\n\
         硬约束：拆解不超过 3 个子任务，每个子任务必须独立可完成，涉及路径互不重叠。\n\
         自评必填：必须逐条说明设计如何自证验收标准，空自评会被机械校验直接打回。\n\n\
         【方向】{方向}\n【现稿】{设计}\n【拆解】{拆解:?}\n【涉及路径】\n{涉及路径}\n【评审意见】\n{意见}",
        方向 = 要求.方向,
    ...
    for 值 in 提取JSON候选们(回复) {
        match 值["有意见"].as_bool() {
            Some(true) => {
                ...
                if !意见.is_empty() {
                    return 评审意见 { 角度, 有意见: true, 意见 };
                }
            }
            Some(false) => return 评审意见 { 角度, 有意见: false, 意见: String::new() },
            None => continue,
        }
    }
    评审意见 { 角度, 有意见: false, 意见: String::new() }
}
...
fn 解析设计方案(要求id: &str, 回复: &str) -> Option<设计方案> {
    for 值 in 提取JSON候选们(回复) {
        ...
        let mut 拆解 = Vec::new();
        for 项 in 拆解数组 {
            let Some(目标) = 项["目标"].as_str() else { continue };
            let 目标 = 目标.trim();
            if 目标.is_empty() {
                continue;
            }
    ...
    warn!(要求id, 字数 = 回复.chars().count(), "设计JSON候选均不完整，回退模板");
    None
}
...
mod 测试 {
    ...
    fn think块含示例花括号仍能解析出真设计() {
        ...
        assert!(方案.设计.contains("观览-查询-殿"), "设计应为后段真 JSON：{}", 方案.设计);
        assert_eq!(方案.拆解.len(), 1);
    }
    ...
    #[test]
    fn 分级判定_新能力为复杂() {
        use crate::类型_定义_殿::{约束, 阶段, 要求来源, 要求书, 要求类别, 要求状态};
        let 要求 = 要求书 {
            id: "要求-1".to_string(),
            来源: 要求来源::界主,
            想法id: None,
    ...
    fn 分级判定_简单任务为单轮() {
        ...
        let 要求 = 要求书 {
            ...
            类别: 要求类别::维护,
            验收标准: "y".to_string(),
            约束: 约束 { 涉及路径: vec!["乾坤/呈现-域/命令操作-府/a.rs".to_string()], ..约束::default() },
            状态: 要求状态::待领,
            确认意见: None,
            验收: None,
            版本: None,
    ...
    fn 分级判定_涉及路径多或跨府为复杂() {
        ...
        let 造 = |路径们: Vec<&str>| 要求书 {
            ...
            类别: 要求类别::功能,
            验收标准: "y".to_string(),
            约束: 约束 { 涉及路径: 路径们.into_iter().map(|p| p.to_string()).collect(), ..约束::default() },
            状态: 要求状态::待领,
            确认意见: None,
            验收: None,
            版本: None,
        };
        assert!(是复杂任务(&造(vec!["a/域/府-1/x.rs", "a/域/府-1/y.rs", "a/域/府-1/z.rs"])), "涉及路径≥3 应复杂");
        assert!(是复杂任务(&造(vec!["a/域/府-1/x.rs", "b/域/府-2/y.rs"])), "跨府应复杂");
        assert!(!是复杂任务(&造(vec!["a/域/府-1/x.rs", "a/域/府-1/y.rs"])), "同府两条应简单");
    }
    ...
    fn 实况_LLM设计不再回退模板() {
        ...
        assert!(!配置.密钥.is_empty(), "缺少 LLM_API_KEY（检查 WORLD_WORKSPACE_ROOT 与 .env）");
        let 要求 = 要求书 {
            id: "实况-1".to_string(),
            来源: 要求来源::界主,
            想法id: None,
            阶段: 阶段::甲,
            方向: "新增『世界 纪元』只读命令，按公历年份推算干支纪年与生肖并格式化输出".to_string(),
            类别: 要求类别::新能力,
            验收标准: "新增只读命令并接线到兜底分发，干支/生肖推算正确，单元测试通过".to_string(),
            约束: 约束 {
                涉及路径: vec![
                    "乾坤/呈现-域/命令操作-府/观览-查询-殿/世界-观览-阁/流式-纪元-园/流式纪元.rs".to_string(),
        ...
        println!("【实况】LLM 设计成功：{}（拆解 {} 项）", 方案.设计, 方案.拆解.len());
    }
}
