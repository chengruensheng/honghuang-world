...
/// → 任务全程注入固定规则段 → drop（任务结束）写失效标记清理（读链头集注入时过滤失效）。
...
impl 临时天道守卫 {
    ...
    fn 尝试注入(存储: &shihai_fu::模型存储, 想法内容: &str) -> Option<Self> {
        ...
        // 独立实体键：任务结束写失效标记时，实体键级失效判定可跳过该键全部旧记录（§14.19 清理语义）。
        记录.实体键 = "临时天道".to_string();
        if let Err(错误) = 存储.写记录(&记录) {
            warn!(错误 = %错误, "临时天道写入失败");
            return None;
        }
        info!(规则 = %规则文本, "临时天道已写入（任务级规则）");
        Some(临时天道守卫 {
            存储: 存储.clone()
        })
    }
...
pub fn 运行一轮(
    ...
    // 任务全程进固定规则段；守卫 drop（任务结束）写失效标记清理。
    let _临时天道 = 临时天道守卫::尝试注入(存储, &想法.内容);
    let mut 总用量 = 解析用量;
    // 落点护栏：涉及路径为空且非审验/核查类 → 拒绝放行（防模型乱闯烧预算）。
