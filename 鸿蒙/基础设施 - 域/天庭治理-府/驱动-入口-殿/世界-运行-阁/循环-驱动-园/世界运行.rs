        ...
        let 回执们 = std::thread::scope(|作用域| {
            ...
            let 句柄们: Vec<_> = 任务们
                ...
                .map(|(序号, 任务)| {
                    ...
                    let 背景 = &背景;
                    let 现状 = &执行现状;
                    作用域.spawn(move || 调度.派遣执行(&任务id, 任务, 背景, 现状))
                })
                .collect();
            句柄们
                .into_iter()
...
fn 打回撤销产物(产物们: &[crate::产物条目], 工作区: &shihai_fu::工作区) -> Result<usize, String> {
    // 优先回滚垫：恢复世界写前状态（含未存档的界主/助手改动），比版本快照精确——
    // 版本快照不含未存档改动，按快照恢复会覆盖它们。
    // 撤销全部 部分失败会保留失败组存档供追溯；全部成功则清空残档——防存档组跨轮
    // 累积、下次 `撤销全部` 误撤销旧组污染盘面（2026-08-17 打回残留实锤）。
    let 垫 = shihai_fu::回滚垫::在工作区(工作区);
    match 垫.撤销全部() {
        Ok(恢复) => {
            if let Err(清理错误) = 垫.清理全部() {
...
mod 测试 {
    ...
    fn 打回撤销产物_回溯最近含该文件的快照() {
        ...
        let 撤销数 = 打回撤销产物(&[产物条目 { 路径: "乾坤/甲.rs".to_string(), 类别: "代码".to_string(), 字节数: 0, 变化类型: "修改".to_string() }], &shihai_fu::工作区::新(&根)).unwrap();
        assert_eq!(撤销数, 1);
    ...
    fn 打回撤销产物_全部版本不含则删除新增() {
        ...
        let 撤销数 = 打回撤销产物(&[产物条目 { 路径: "乾坤/乙.rs".to_string(), 类别: "代码".to_string(), 字节数: 0, 变化类型: "新增".to_string() }], &shihai_fu::工作区::新(&根)).unwrap();
        assert_eq!(撤销数, 1);
        assert!(!工作区文件.exists());
        let _ = std::fs::remove_dir_all(&根);
    }
    ...
    fn 打回撤销产物_取最近版本快照() {
        ...
        let 撤销数 = 打回撤销产物(&[产物条目 { 路径: "乾坤/丙.rs".to_string(), 类别: "代码".to_string(), 字节数: 0, 变化类型: "修改".to_string() }], &shihai_fu::工作区::新(&根)).unwrap();
        assert_eq!(撤销数, 1);
    ...
    fn 打回撤销产物_空产物无事发生() {
        ...
        std::fs::create_dir_all(根.join(".上下文/版本库/版本-v1/源码-快照")).unwrap();
        let 撤销数 = 打回撤销产物(&[], &shihai_fu::工作区::新(&根)).unwrap();
        assert_eq!(撤销数, 0);
        let _ = std::fs::remove_dir_all(&根);
    }
