...
fn 读审验标准() -> Option<String> {
    ...
    let 清单 = 记录们
        .iter()
        .filter(|记录| !记录.失效 && 记录.内容.contains("审验标准"))
        .last()?
        .内容
        .clone();
    Some(清单)
...
/// 六准圣分维独立审验：6 次独立模型调用并发（世界内部并发是正确提速路径，缺陷 5 结论），
/// 任意准圣失败仅记 warn 不影响其他准圣。
/// 并发安全：各工作线程内先 进入观测(验收, 要求id)（线程本地上下文，模型调用按此关联
/// 要求id 与角色），单准圣审验 其余逻辑纯读共享引用（要求书/产物们/配置均 Sync）。
pub fn 六准圣审验(
    要求书: &要求书,
    产物们: &[产物条目],
    配置: &模型配置,
