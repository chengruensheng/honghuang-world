...
mod 测试 {
    ...
    /// 流水线：注销句柄 drop 后长度归零，注册表真移除（与事件总线的「注销句柄_丢弃后真移除」并列，
    /// 区别：本测试针对 `流水线<T>::长度` 而非 `事件总线::监听数`）。
    #[test]
    fn 流水线_注销句柄丢弃后长度归零() {
        let mut 链 = 流水线::<u32>::新();
        let 句柄 = 链.注册(|_| Ok(()));
        assert_eq!(链.长度(), 1, "注册后应有 1 个监听器");
        drop(句柄);
        assert_eq!(链.长度(), 0, "drop 注销句柄后应从注册表真移除");
    }
    ...
    /// 工具流水线全链走通：预执行 → 守卫放行 → 执行 → 后执行，四段都产生可观察效果。
    /// 预执行给请求加标记；守卫看到标记则放行；执行基于标记产出文本；后执行追加后缀。
    #[test]
    fn 工具流水线_全链走通() {
        struct 见标记则放行;
        impl 守卫 for 见标记则放行 {
            fn 裁决(&self, 请求: &工具请求) -> 裁决 {
                if 请求.涉及路径.iter().any(|p| p == "预执行已跑") {
                    裁决::放行
                } else {
                    裁决::拒绝("预执行未跑，守卫不放行".to_string())
                }
            }
        }
        let mut 链 = 工具流水线::新(|请求| {
            let 文本 = 请求.涉及路径.join(",");
            Ok(工具结果::新(文本))
        });
        // 预执行：给请求加标记
        let _预 = 链.预执行注册(|请求| {
            请求.涉及路径.push("预执行已跑".to_string());
            Ok(())
        });
        // 守卫：见标记则放行
        链.加守卫(std::sync::Arc::new(见标记则放行));
        // 后执行：追加后缀
        let _后 = 链.后执行注册(|结果| {
            结果.文本.push_str("+后执行已跑");
            Ok(())
        });
        let 结果 = 链.执行(&造请求("读文件")).unwrap();
        assert_eq!(
            结果.文本, "预执行已跑+后执行已跑",
            "全链应走通：预执行加标记→守卫放行→执行产出标记→后执行追加后缀"
        );
    }

    /// 流水线<T>：注销句柄 drop 后监听器长度从 1 恢复为 0；
    /// 多注册场景下逐个 drop 验证长度阶梯递减（强化计数与移除耦合）。
    #[test]
    fn 流水线_注销句柄_drop_恢复() {
        let mut 链 = 流水线::<u32>::新();
        let 句柄 = 链.注册(|_载荷| Ok(()));
        assert_eq!(链.长度(), 1, "注册后应有一个监听器");
        drop(句柄);
        assert_eq!(链.长度(), 0, "drop 句柄后监听器应被移除");

        // 多注册场景：阶梯递减 3→2→1。
        let _甲 = 链.注册(|_| Ok(()));
        let 乙 = 链.注册(|_| Ok(()));
        let 丙 = 链.注册(|_| Ok(()));
        assert_eq!(链.长度(), 3, "再注册三个后应累计为 3");
        drop(丙);
        assert_eq!(链.长度(), 2, "drop 丙后应为 2");
        drop(乙);
        assert_eq!(链.长度(), 1, "drop 乙后应为 1");
    }
}
