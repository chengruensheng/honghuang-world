...
$返工会话 = 0        # 本会话时间窗内，关联任务的结构化状态流转曾出现「待修复」（准圣打回）
$分层事件数 = @{}    # 按事件自带的结构化层标签汇总（不靠文本推断所属层）
...
$任务终态 = @{}
$任务返工记录 = @{}   # 任务级结构化打回记录（状态流转中「新状态=待修复」的条目，含时间）
foreach ($任务 in $看板全部) {
    $任务终态[$任务.id] = $任务.status
    # 打回判定改取结构化来源（任务模型自带的状态流转记录），
    # 不再依赖对事件内容做中文文本匹配——后者既会把「模型讨论中提及该词」误判成返工，
    # 也会因事件内容有 200 字符截断而漏判真打回。
    $任务返工记录[$任务.id] = @($任务.状态历史 | Where-Object { [string]$_.新状态 -eq '待修复' })
}
...
foreach ($会话 in $会话们) {
    ...
    $本会话返工 = $false       # 每会话重置：准圣打回（任务状态历史含「待修复」）或隐式修复
    $本会话测试动作序列 = @()
    ...
    foreach ($事件 in $事件们) {
        ...
        $事件类型 = [string]$事件.类型
        $工具名 = [string]$事件.工具名
        # 按层计数：直接读事件自带的结构化「层级」标签（服务端由承接状态经权威映射得出），
        # 无需再从事件内容里猜该事件属于哪一层
        $层 = [string]$事件.层级
        if (-not [string]::IsNullOrEmpty($层)) { $分层事件数[$层] = [int]$分层事件数[$层] + 1 }
        # 测试动作：运行命令工具且命令含 cargo test / cargo test -p ...
        if ($事件类型 -eq "工具调用" -and $工具名 -eq "运行命令" -and $内容 -match "cargo\s+test") {
            $本会话有测试动作 = $true
            $本会话测试动作序列 += $内容
        }
        # 显式打回（准圣「待修复」）的判定已移出事件循环：改读任务的结构化状态历史（见循环后），
        # 不再对事件内容做中文文本匹配
    ...
    # 显式打回（准圣「待修复」）：读关联任务的结构化状态流转记录，并以**本会话时间窗**限定——
    # 避免同一任务被多会话关联（恢复/分叉驱动）时，把上一会话的打回重复计入本会话。
    # 时间单位一致：会话 创建时间 与 状态流转 时间 均为秒级时间戳。
    $本会话起始秒 = [long]$会话.创建时间
    foreach ($tid in @($会话.任务id列表)) {
        if (-not $任务返工记录.ContainsKey($tid)) { continue }
        $窗内打回 = @($任务返工记录[$tid] | Where-Object { [long]$_.时间 -ge $本会话起始秒 })
        if ($窗内打回.Count -gt 0) { $本会话返工 = $true }
    }
...
Write-Host ("  分层事件数 = {0}（事件自带层标签按层汇总；「未标注」为改造前落盘的旧会话）" -f ((@(($分层事件数.GetEnumerator() | Sort-Object Key | ForEach-Object { "$($_.Key):$($_.Value)" })) -join '、')))
Write-Host ""
