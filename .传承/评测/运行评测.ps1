...
foreach ($任务 in $任务清单) {
    ...
    if ($真实) {
        ...
        $终态 = $当前终态
        # 流程停在「待道祖澄清」等人裁决属**非终态**，会让看板无法自然清空，下一轮评测因「脏看板」
        # 拒绝启动。评测方代用户扮演道祖给出终止结论（继续=false → 已取消），使任务进终态、看板可复用。
        # 这不掩盖事实：自述通过=false、外部判据结论照常入统计，行为完整留痕。
        # 结论文案必须**中性**：澄清既可能源于「需求无解的正确拒绝」，也可能源于「有解任务被卡住」，
        # 脚本无从分辨，不得臆断原因（第 3 轮实测：两个有解的正例任务同样卡在此态）。
        if ($自动终止卡住任务 -and $终态集合 -notcontains $终态 -and $终态 -eq '待道祖澄清') {
            try {
                $澄清 = 调用Json POST "/api/board/$发布任务号/clarify" @{ 结论 = '评测方代道祖终止：任务停滞于「待道祖澄清」非终态，为使看板可复用而终态化；真实原因见该任务阶段文档，脚本不作判定'; 继续 = $false }
                存原文 ("任务-{0}-澄清.json" -f $任务.id) $script:最近原文
                if ($null -ne $澄清) { $终态 = [string]$澄清.新状态 }
                Write-Host "  卡住处置：任务停在「待道祖澄清」，评测方代道祖终止 → $终态" -ForegroundColor Yellow
