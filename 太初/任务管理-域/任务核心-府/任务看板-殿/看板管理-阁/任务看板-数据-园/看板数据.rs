...
impl TaskBoard {
    ...
    /// 仅标记：写回退记录（标记问题）但不改变任务状态与当前层级。
    /// 用于「失败恢复」的「仅标记」恢复方式——用户已知失败、暂不回退，仅留痕。
    /// 返回累计回退次数（含本次标记）。
    pub fn 标记回退(&mut self, 任务id: u64, 根源层级: 五行层级, 错误描述: &str) -> Result<u32> {
        let 累计 = {
            let task = self.任务集.get_mut(&任务id).ok_or_else(|| Error::任务不存在(任务id))?;
            let 累计 = task.回退来源.as_ref().map(|r| r.回退次数).unwrap_or(0) + 1;
            task.回退来源 = Some(回退记录::新(
                当前时间戳(),
                五行层级::金,
                根源层级,
                回退原因(根源层级),
...
/// 回退/标记原因归类（按目标层级；木的「超限升级」语境与定向回退一致）
fn 回退原因(层级: 五行层级) -> &'static str {
    match 层级 {
