...
#[cfg(test)]
mod 测试 {
    use moxing_fu::{对话消息, 工具定义, 模型回复, 模型配置};
    ...
    fn 解析回复空内容返回错误() {
        ...
        assert!(
            解析回复(空串).unwrap_err().contains("空内容"),
            "空串应报空内容错误"
        );
        let 纯空白 = r#"{"choices":[{"message":{"content":"  \n  "}}]}"#;
        assert!(
            解析回复(纯空白).unwrap_err().contains("空内容"),
            "纯空白应报空内容错误"
        );
    ...
    fn 解析工具回复取调用带标识() {
        ...
        assert!(
            matches!(&回复, 模型回复::工具调用(内容, 调用们) if 内容 == "<think>思考</think>" && 调用们.len() == 1 && 调用们[0].名字 == "落盘文件" && 调用们[0].标识 == "1")
        );
    }
    ...
    fn 解析工具回复取调用() {
        ...
        let 回复 = 解析工具回复(文本).unwrap();
        assert!(
            matches!(&回复, 模型回复::工具调用(_, 调用们) if 调用们.len() == 1 && 调用们[0].名字 == "落盘文件")
        );
    }
