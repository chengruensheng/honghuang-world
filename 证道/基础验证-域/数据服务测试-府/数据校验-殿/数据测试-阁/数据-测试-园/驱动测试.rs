...
mod tests {
    ...
    /// （空看板时驱动会瞬间返回空闲，仅靠 mock 延迟无法保证运行中窗口 —— 同类偶发根因）
    #[tokio::test]
    ...
    /// P0.2：阶段产出首次非法 JSON → 回喂 serde 错误重试 → 第二次合法成功推进
    #[tokio::test]
    async fn 驱动台_解析失败回喂重试成功() {
        let (状态, 看板) = 驱动状态();
        let 对话器 = Arc::new(模拟对话器::新(vec![
            模型响应 { 内容: Some("{\"边界定义\": 123}".into()), 工具调用: vec![], 思考: None },
            模型响应 { 内容: Some(设计样例().into()), 工具调用: vec![], 思考: None },
        ]));
        装配驱动器(&状态, &看板, 对话器);
        发布任务(&状态, "解析重试任务").await;
        assert!(状态.看板驱动台.等待完成(5000), "发布后自动驱动应在超时内完成");

        let 看板守卫 = 看板.lock().expect("看板锁");
        let 任务 = 看板守卫.查询(1).expect("任务应存在");
        assert_eq!(任务.status, TaskStatus::待大罗金仙实现, "重试成功后应推进到实现");
        let 设计 = 任务.设计文档.as_ref().expect("设计文档应写入");
        assert_eq!(设计.契约[0].契约名, "测试契约", "重试后的设计文档应正确解析");
    }

    /// P0.2：阶段产出连续非法 JSON（重试 2 次仍失败）→ 明确报错，任务停在待承接
    #[tokio::test]
    async fn 驱动台_解析重试两次仍失败明确报错() {
        let (状态, 看板) = 驱动状态();
        let 对话器 = Arc::new(模拟对话器::新(vec![
            模型响应 { 内容: Some("{\"边界定义\": 123}".into()), 工具调用: vec![], 思考: None },
            模型响应 { 内容: Some("{\"边界定义\": 123}".into()), 工具调用: vec![], 思考: None },
        ...
        装配驱动器(&状态, &看板, 对话器);
        发布任务(&状态, "解析失败任务").await;
        assert!(状态.看板驱动台.等待完成(5000), "驱动应在超时内结束（失败也结束）");

        let 看板守卫 = 看板.lock().expect("看板锁");
        let 任务 = 看板守卫.查询(1).expect("任务应存在");
        assert_eq!(任务.status, TaskStatus::待圣人设计, "重试失败后任务应停在待承接");
        drop(看板守卫);

        let 摘要 = 状态.看板驱动台.当前状态();
        let 结果 = 摘要.最近结果.expect("应有最近结果（失败）");
        assert!(
            结果.contains("解析失败") || 结果.contains("无法解析") || 结果.contains("设计文档"),
            "应说明解析失败: {结果}"
        );
    }
}
