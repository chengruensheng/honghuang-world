...
mod tests {
    ...
    fn 完整链路响应() -> Vec<模型响应> {
        vec![
            ...
            模型响应 { 内容: Some(清理样例().into()), 工具调用: vec![] },
        ]
    }
    ...
    async fn 驱动到空闲_完整六层链自动流转到清理完成() {
        ...
        let 摘要 = 状态.看板驱动台.当前状态();
        assert_eq!(摘要.最近结果.expect("应有汇总"), "共推进 4 轮", "drain 应推进 实现/验收/终审/清理 四轮");
        let 阶段 = 摘要.最近阶段.expect("应有最近阶段");
        assert_eq!(阶段.类型, "阶段完成");
    ...
    async fn 驱动事件流_多轮驱动记录完整事件() {
        ...
        assert_eq!(事件.len(), 6, "应记录 设计/实现/验收/终审/清理 阶段完成 + 空闲");
        assert_eq!(事件[0].序号, 1);
        assert_eq!(事件[0].类型, "阶段完成");
        assert_eq!(事件[0].新状态.as_deref(), Some("待大罗金仙实现"));
        assert_eq!(事件[3].新状态.as_deref(), Some("待清理"));
        assert_eq!(事件[4].新状态.as_deref(), Some("清理完成"));
        assert_eq!(事件[5].类型, "空闲");
    }
    ...
    async fn 驱动事件接口_事件增量与状态() {
        ...
        assert_eq!(响应.事件.len(), 6, "全量事件 6 条");

        let Json(部分) = 看板驱动事件接口(State(状态.clone()), Query(事件游标 { since: Some(3) })).await;
        assert_eq!(部分.事件.len(), 3, "since=3 应剩 3 条");
