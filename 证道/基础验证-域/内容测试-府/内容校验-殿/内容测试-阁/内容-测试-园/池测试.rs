...
mod tests {
    ...
    #[test]
    fn 池_模板清单_九家含地址() {
        let 模板们 = hm_content::热门模板();
        assert!(模板们.len() >= 9);
        let deepseek = 模板们.iter().find(|t| t.名称 == "deepseek").unwrap();
        assert!(deepseek.地址.contains("api.deepseek.com"));
        assert!(!deepseek.默认模型.is_empty());
        let json = serde_json::to_string(&模板们).unwrap();
        assert!(!json.contains("api_key") && !json.contains("密钥"), "模板不得含密钥字段: {json}");
    }

    #[test]
    fn 池_模板模型_目录命中零网络() {
        let 模型 = hm_content::模板模型("deepseek").unwrap();
