...
mod tests {
    ...
    #[test]
    fn 扫描器_不存在路径报错() {
        let 结果 = Rust扫描器::新().扫描(&std::path::Path::new("Z:/不存在的扫描根路径"));
    ...
    #[test]
    fn 扫描器_真实洪荒靶子() {
        let 根 = std::path::Path::new(r"F:\洪荒 - 世界");
        if !根.exists() {
            return; // 靶子缺失时跳过（不视为失败）
        }
    ...
    fn 扫描器_技术栈去重() {
        ...
        std::fs::write(根.join("Cargo.toml"), "[workspace]\nmembers = [\"丙\"]\n").expect("写Cargo");
        std::fs::write(根.join("丙").join("Cargo.toml"), "[package]\nname = \"丙\"\n[dependencies]\nserde = \"1\"\nserde = \"1\"\n[dependencies.tokio]\nversion = \"1\"\n").expect("写丙Cargo");
