...
mod tests {
    ...
    fn 引号内脱字符不被误拦() {
        let 根 = 准备工作区("引号脱字符");
        let 执行器 = 本地执行器::new(&根);
        // cmd 引号态既不解释也不剥离 ^：findstr /v "^$" 这类正当写法不得被当转义绕过拦下
        // （2026-09-14 实证：镜像验证任务因「含 ^ 一律拦」无法运行 cargo build/test）
        let 输出 = 执行器
            .运行命令("echo \"^$\"")
            .expect("引号内 ^ 不应被拦截");
        assert!(输出.contains("^$"), "cmd 引号态保留 ^，实际: {输出}");
        let _ = std::fs::remove_dir_all(&根);
    }
    ...
    fn 引号外脱字符报错指明转义符() {
        let 根 = 准备工作区("转义报错文案");
        let 执行器 = 本地执行器::new(&根);
        // 报错须点明真实原因（引号外 ^），否则模型会误判成「命令不在白名单」
        let 错误 = 执行器
            .运行命令("d^el a.txt")
            .expect_err("引号外 ^ 应拦截")
            .to_string();
        assert!(错误.contains("转义符"), "应指明转义符原因: {错误}");
        let _ = std::fs::remove_dir_all(&根);
    }
