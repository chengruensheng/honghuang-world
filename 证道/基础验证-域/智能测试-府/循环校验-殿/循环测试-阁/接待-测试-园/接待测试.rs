...
mod tests {
    ...
    fn 对齐总结响应(标题: &str, 描述: &str, 场景: &str, 优先级: &str) -> 模型响应 {
        ...
        .to_string();
        模型响应 { 思考: None, 内容: None, 工具调用: vec![工具调用("对齐总结", &参数)] }
    }

    /// 构造「追问澄清」模型响应
    fn 追问澄清响应(问题: &str) -> 模型响应 {
        let 参数 = serde_json::json!({ "问题": 问题 }).to_string();
        模型响应 { 思考: None, 内容: None, 工具调用: vec![工具调用("追问澄清", &参数)] }
    }

    /// 构造「闲聊」模型响应
    fn 闲聊响应(回复: &str) -> 模型响应 {
        let 参数 = serde_json::json!({ "回复": 回复 }).to_string();
        模型响应 { 思考: None, 内容: None, 工具调用: vec![工具调用("闲聊", &参数)] }
    }
    ...
    fn 道祖接待_对齐总结缺标题返回错误() {
        ...
        let 模拟 = Arc::new(模拟对话器::新(vec![模型响应 { 思考: None,
            内容: None,
