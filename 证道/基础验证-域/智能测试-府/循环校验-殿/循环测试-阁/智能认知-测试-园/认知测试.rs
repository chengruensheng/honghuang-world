...
mod tests {
    ...
    #[test]
    fn 认知注入_临时规则注入出现在初始注入() {
        let (图谱, 心智, 上下文) = 辅助三态();
        let 注入 = 认知注入::新(图谱, 心智, 上下文);
        注入.注入临时规则(vec!["不得修改 Cargo.toml".into(), "输出文件一律放 工作区/产出/".into()]);

        let 段 = 注入.初始注入("任意任务");
        assert!(段.contains("【临时·任务规则】"), "初始注入应含临时规则段头，实际: {段}");
        assert!(段.contains("1. 不得修改 Cargo.toml"), "应含编号规则一，实际: {段}");
        assert!(段.contains("2. 输出文件一律放 工作区/产出/"), "应含编号规则二，实际: {段}");
    }
    ...
    fn 认知注入_临时规则清除后不再现() {
        let (图谱, 心智, 上下文) = 辅助三态();
        let 注入 = 认知注入::新(图谱, 心智, 上下文);
        注入.注入临时规则(vec!["禁止使用 unwrap".into()]);
        注入.清除临时规则();

        assert!(
            !注入.初始注入("任意任务").contains("【临时·任务规则】"),
            "清除后初始注入不应含临时规则段"
        );
        // 重复清除应幂等不 panic
        注入.清除临时规则();
    ...
    #[test]
    fn 认知注入_临时规则空白条目清洗且重复注入覆盖() {
        let (图谱, 心智, 上下文) = 辅助三态();
        let 注入 = 认知注入::新(图谱, 心智, 上下文);
        注入.注入临时规则(vec!["  ".into(), String::new(), "有效规则甲".into()]);
        // 重复注入（任务重承接）应整组覆盖而非叠加
        注入.注入临时规则(vec!["有效规则乙".into()]);

        let 段 = 注入.初始注入("任意任务");
        assert!(段.contains("有效规则乙"), "重复注入后应有新规则，实际: {段}");
        assert!(!段.contains("有效规则甲"), "旧规则应被覆盖，实际: {段}");
        assert!(!段.contains("1.  "), "空白条目应被清洗（无空规则占位），实际: {段}");
    }

    #[test]
    fn 认知注入_无临时规则时初始注入无该段() {
        let (图谱, 心智, 上下文) = 辅助三态();
        let 注入 = 认知注入::新(图谱, 心智, 上下文);
        assert!(
            !注入.初始注入("任意任务").contains("【临时·任务规则】"),
    ...
    fn 智能体_未装配认知消息不变() {
        let 对话器 = Arc::new(记录对话器::新(vec![
            模型响应 { 思考: None, 内容: Some("直接答复".into()), 工具调用: vec![] },
        ]));
    ...
    fn 认知注入_答复注入有池组装自然语言() {
        let (图谱, 心智, 上下文) = 辅助三态();
        let 注入 = 认知注入::新(图谱, 心智, 上下文);
        let 对话器 = Arc::new(记录对话器::新(vec![模型响应 { 思考: None,
